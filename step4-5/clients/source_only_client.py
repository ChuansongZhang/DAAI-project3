...
from clients import GeneralClient
...
class SourceOnlyClient(GeneralClient):
    ...
    def calc_loss_and_output(self, images, labels):
        ...
        if self.args.model in ('bisenetv2',):

            if "div" in self.args.sourceonly_loss:

                outputs = self.model(images)[0]
                with torch.no_grad():
                    outputs_old = self.teacher_kd_model(images)[0] if self.args.lambda_kd > 0 else None
    ...
    def calc_test_loss(self, outputs, labels):
        ...
        elif self.args.model in ('bisenetv2',) and "div" in self.args.fda_loss:
            lovasz_loss = self.test_reduction(self.test_criterion(outputs, labels), labels)
            entropy_loss = self.entropy_loss(outputs)
            return lovasz_loss + entropy_loss
        else:
    ...
    def __get_criterion_and_reduction_rules(self, use_labels=False, criterion = None):
        ...
        if not (use_labels or self.source_client):
            if self.args.sourceonly_loss == "lovasz_entropy_div":
                criterion = loss_fn = LovaszLoss(lambda_selftrain=self.args.lambda_selftrain, **shared_kwargs)
            elif self.args.sourceonly_loss == "selftrain_div":
