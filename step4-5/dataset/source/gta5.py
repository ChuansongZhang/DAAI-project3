...
class GTA5(SourceDataset):
    ...
    images_dir = '/content/drive/MyDrive/DATASET/GTA5/images'
    target_dir = '/content/drive/MyDrive/DATASET/GTA5/labels'
    dataset_dir = '/content/drive/MyDrive/DATASET/GTA5'
    ds_type = 'supervised'
    ...
    def __init__(self, root, transform=None, test_transform=None, mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5), cv2=False,
        ...
        super().__init__(root, transform=transform, test_transform=test_transform, mean=mean, std=std, cv2=cv2,
                         split=split)
        if split == 'train':
          item_list_filepath = os.path.join(self.dataset_dir, f'{split}.txt')
