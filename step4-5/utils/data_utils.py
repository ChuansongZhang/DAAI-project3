...
class DatasetHandler(object):
    ...
    def __init__(self, args, writer):
        ...
        self.source_stats = {}
        self.target_stats = {}
        self.DATA_DIR = '/content/drive/MyDrive/DATASET'
    ...
    @staticmethod
    def __get_paths(DATA_DIR, dataset, clients_type, framework, partition, train_data_dir=None, test_data_dir=None ):
        if dataset == 'gta5':
          clients_type = ''
          train_data_dir = os.path.join(DATA_DIR, dataset, clients_type, 'train.txt')

        if framework == 'centralized':

          # data list directory
          train_data_dir = os.path.join(DATA_DIR, dataset, framework, 'train', f'train_{partition}')
          test_data_dir = os.path.join(DATA_DIR, dataset, framework, 'test', f'test_{partition}')
        
          # retrieve the images name and labels name
        elif framework == 'federated':
          train_data_dir = os.path.join(DATA_DIR, dataset, framework, clients_type, 'train',
                                        f'train_{partition}')
          test_data_dir = os.path.join(DATA_DIR, dataset, framework, clients_type, 'test',
            f'test_{partition}')
    ...
    def __source_dataset_init(self):
        ...
        train_ds = dataset(self.DATA_DIR, transform=train_transform, test_transform=test_transform,
                           split='train')
        test_ds = []
        # return train_ds [(image, label)] list(tuple)
    ...
    def __gen_ds(self, paths, dataset_name, dataset, train_transform, test_transform, split='train', hp_filtered=False):
        if dataset_name == 'cityscapes':
            return dataset(paths, self.DATA_DIR, transform=train_transform, test_transform=test_transform,
    ...
    def __target_dataset_init(self):
        ...
        train_data_dir, test_data_dir = self.__get_paths(self.DATA_DIR, dataset_name, self.args.clients_type, framework=self.args.framework,
        ...
        # train_users = [''] tets_users = [''test_user']
