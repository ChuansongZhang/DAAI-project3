...
class CustomWandbLogger(pytorch_lightning.loggers.WandbLogger):
        ...
        while True:
            try:
                super().__init__(
                    ...
                    project=self.get_project_name(args.framework, args.source_dataset, args.target_dataset, args.partition),
    ...
    def get_job_name(self, args):
        ...
        if not args.pretrain:
            job_name += f"lr{args.lr}_bs{args.total_batch_size}_rs{args.random_seed}_ts_{args.teacher_step}_fda_b{args.fda_b}_{args.name}"
        else:
             job_name += f"lr{args.lr}_bs{args.total_batch_size}_rs{args.random_seed}_fda_b{args.fda_b}_{args.name}"
             
        self.job_name = job_name

        return job_name

    def get_sourceonly_pretrained_model(self):
        ...
        def restore(run_id, best: bool):
            try:
                # print('1')
                ckpt_path = '/'.join(['checkpoints', self.args.framework, f'{self.args.source_dataset}_TO_{self.args.target_dataset}_pretrained',
                                 self.args.fw_task, f'{self.args.partition}_{self.args.clients_type}',
                                'pretrained_server{}_{}.ckpt'.format("_best" if best else "", self.args.lr)])
                # print(ckpt_path)
                # print('2')
        ...
        if self.args.load_SourceOnly_id is not None:
            print(f"Loading previous model from {self.args.load_SourceOnly_id}")
            ckpt = restore(self.args.load_SourceOnly_id, self.args.load_SourceOnly_best)
            if ckpt is not None:
                return ckpt
        ...
        for old_run in wandb.Api().runs(path=wandb_path):
            if old_run._attrs["group"] == group_name:
                ...
                if not diff and dict_eq(new_run_params, old_run_params):
                    print(f"same SourceOnly pretraining found, loading previous model from {old_run.id}")
                    ckpt = restore(old_run.id)
                    if ckpt is not None:
                        return ckpt

        print(f"No SourceOnly pre-trained ckpt has been found. Starting pre-training from scratch...")
    ...
    def get_fda_pretrained_model(self):
        ...
        def restore(run_id, best: bool):
            try:
                ckpt_path = '/'.join(
                    ['checkpoints', self.args.framework, f'{self.args.source_dataset}_TO_{self.args.target_dataset}_pretrained',
                                 self.args.fw_task, f'{self.args.partition}_{self.args.clients_type}',
                                  f'pretrained_server{"_best" if best else ""}_lr{self.args.lr}_fda_b{self.args.fda_b}.ckpt'])
                # print(ckpt_path)
                # print('2')
                wandb.restore(ckpt_path, run_path=f"{wandb_path}/{run_id}", replace=True, root=os.getcwd())
                # print('3')
    ...
    @staticmethod
    def get_project_name(framework, source_dataset, target_dataset, partition):
      if source_dataset:
        return f"{framework}_{source_dataset}_{target_dataset}_{partition}"
      return f"{framework}_{target_dataset}_{partition}"
