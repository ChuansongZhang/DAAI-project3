...
from utils import DatasetHandler, dynamic_import
...
class GeneralTrainer(object):
    ...
    def __init__(self, args, writer, device, rank, world_size):
        ...
        self.ckpt_path = os.path.join('checkpoints', args.framework, f'{args.source_dataset}_To_{args.target_dataset}',
                                      args.fw_task, f'{self.args.partition}_{self.args.clients_type}', f'{self.writer.wandb.get_job_name(args)}_{self.args.wandb_id}.ckpt')
    ...
    def __load_wandb(self):
        run_path = os.path.join(
            ...
            self.writer.wandb.get_project_name(self.args.framework, self.args.source_dataset, self.args.target_dataset, self.args.partition),
            self.args.wandb_id
        )
    ...
    def __get_plot_sample(self, test_client, sample_id):
        ...
        # print('-----test type---------')
        # print(f'--------\n{type(sample[0][0])},{sample[0][0].shape}')
        # print(f'---------\n{type(sample[0][0].unsqueeze(0))},{sample[0][0].unsqueeze(0).shape}')
        self.model.eval()

        with torch.no_grad():
            sample_pred = test_client.get_test_output(sample[0][0].unsqueeze(0).to(self.device))[0].argmax(dim=1)[0].detach().cpu().numpy()
            # sample_pred = test_client.get_test_output(sample[0][0])[0].argmax(dim=1)[0].detach().cpu().numpy()
        ...
        if hasattr(test_client.criterion, 'get_pseudo_lab'):
            with torch.no_grad():
                pred_torch = test_client.get_test_output(sample[0][0].unsqueeze(0).to(self.device))[0]
