...
class FdaTrainer(TrainerFed,TrainerCent):
    ...
    def __init__(self, args, writer, device, rank, world_size):
        ...
        self.set_client_style_tf_obj() #active or comment for FDA or Source_only
        writer.write(f'Done')
    ...
    def save_pretrained_server(self):
        ...
        torch.save(state, os.path.join(ckpt_path, 'pretrained_server_{}_fda_b{}.ckpt'.format(self.args.lr, self.args.fda_b)))
        self.writer.wandb.save(os.path.join(ckpt_path, 'pretrained_server_{}_fda_b{}.ckpt'.format(self.args.lr, self.args.fda_b)))
        print("Done")
