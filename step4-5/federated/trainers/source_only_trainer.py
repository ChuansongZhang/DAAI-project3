...
from federated.trainers.trainer import Trainer as TrainerFed
from utils import dynamic_import, get_optimizer_and_scheduler, schedule_cycling_lr
from centralized.trainers.trainer import Trainer as TrainerCent


class SourceOnlyTrainer(TrainerFed, TrainerCent):
    ...
    def __init__(self, args, writer, device, rank, world_size):
        ...
        writer.write(f'Initializing direct transfer module in clients...')
        writer.write(f'Done')
    ...
    def set_client_teacher(self, r, model):
        ...
        if r % self.args.teacher_step == 0 and not self.args.teacher_upd_step:
            ...
            self.writer.write(f"round {r + 1}, setting new teacher...")
    ...
    def save_pretrained_server(self):
        ...
        ckpt_path = os.path.join('checkpoints', self.args.framework, f'{self.args.source_dataset}_TO_{self.args.target_dataset}_pretrained',
                                 self.args.fw_task, f'{self.args.partition}_{self.args.clients_type}')
        if not os.path.exists(ckpt_path):
            os.makedirs(ckpt_path)
        torch.save(state, os.path.join(ckpt_path, 'pretrained_server_{}.ckpt'.format(self.args.lr)))
        self.writer.wandb.save(os.path.join(ckpt_path, 'pretrained_server_{}.ckpt'.format(self.args.lr)))
        print("Done")
    ...
    def train(self):
        ...
        if self.ckpt_round == 0:
            ...
            if self.args.load_SourceOnly:
                self.writer.write(f'Checking whether pretrained server model exists...')
                pretrained_ckpt = self.writer.wandb.get_sourceonly_pretrained_model()
        ...
        for r in range(self.ckpt_round, self.args.num_rounds):
            ...
            if self.args.swa_teacher_start != -1 and r + 1 >= self.args.swa_teacher_start:
                ...
                self.swa_teacher_n = (r + 1 - self.args.swa_teacher_start)/self.args.teacher_step
                print(f'swa_teacher_n:{self.swa_teacher_n}')
            ...
            if self.args.teacher_kd_step > 0 and self.args.teacher_kd_mult_factor != -1 and \
                    'source_only' in self.args.fw_task and r % self.args.teacher_kd_mult_step == 0 and r != 0:
