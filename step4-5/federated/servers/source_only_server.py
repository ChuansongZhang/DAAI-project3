...
class SourceOnlyServer(Server):
    ...
    def save_pretrained_server(self, model_state, best: bool):
        ...
        torch.save(state, os.path.join(ckpt_path, 'pretrained_server{}_{}.ckpt'.format("_best" if best else "", self.args.lr)))
        self.writer.wandb.save(os.path.join(ckpt_path, 'pretrained_server{}_{}.ckpt'.format("_best" if best else "", self.args.lr)))
        # should be a filename, not a directory name
        print("Done.")
