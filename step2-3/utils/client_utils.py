    ...
    for i, user in enumerate(users):
        ...
        if args.dataset == 'cityscapes':
            ds = dataset(data=data, transform=train_transform, test_transform=test_transform,
                         ...
                         dom_gen=args.dom_gen, split_name=args.clients_type) 
            # images_1, labels_1 = ds[1]
            # print(images_1)
        ...
        client = client_func(user, ds, model, logger, writer, args, batch_size, world_size, rank, num_gpu,
                             device=device, disable_ddp=disable_ddp, ckpt_path=ckpt_path, name=args.name)
...
def setup_clients(DATA_DIR, args, logger, writer, model, world_size, rank, num_gpu, device=None, ckpt_path=None,
                  disable_ddp=True, train_data_dir=None, test_data_dir=None):
    if args.dataset == 'cityscapes':
        if args.framework == 'centralized':
            train_data_dir = os.path.join(DATA_DIR, args.dataset, args.framework, 'train', f'train_{args.partition}')
            test_data_dir = os.path.join(DATA_DIR, args.dataset, args.framework, 'test', f'test_{args.partition}')

        elif args.framework == 'federated':
            train_data_dir = os.path.join(DATA_DIR, args.dataset, args.framework, args.clients_type, 'train',
                                         f'train_{args.partition}')
            test_data_dir = os.path.join(DATA_DIR, args.dataset, args.framework, args.clients_type, 'test',
             f'test_{args.partition}')
