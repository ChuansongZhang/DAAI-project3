...
def get_group_name(args):
    if args.framework == 'centralized':
        group_name = f"{args.algorithm}"
    else:
        group_name = f"{args.algorithm} + {args.clients_type}"
    if args.dom_gen is not None:
        group_name += f"_{args.dom_gen}"
    return group_name


def get_project_name(framework, dataset, partition):
    return f"{framework}_{dataset}_{partition}"
